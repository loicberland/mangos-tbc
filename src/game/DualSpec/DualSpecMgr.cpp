...
DualSpecMgr& DualSpecMgr::Instance()
{
    static DualSpecMgr instance;
    return instance;
}
void DualSpecMgr::Initialize()
    {
        if (!sWorld.getConfig(CONFIG_BOOL_DUAL_SPEC_ENABLED))
        {
            sLog.outString("DualSpec: Disabled.");
            return;
        }

        sLog.outString("DualSpec: Initializing native DualSpec system...");
        ...
        sLog.outString("DualSpec: Initialized.");
    }

    bool DualSpecMgr::OnUseItem(Player* player, Item* item)
    {
        if (sWorld.getConfig(CONFIG_BOOL_DUAL_SPEC_ENABLED))
        {
            if (player && item)
    ...
    bool DualSpecMgr::OnPreGossipHello(Player* player, Creature* creature)
    {
        if (sWorld.getConfig(CONFIG_BOOL_DUAL_SPEC_ENABLED))
        {
            if (player && creature)
            {
                ...
                const uint32 cost = sWorld.getConfig(CONFIG_UINT32_DUAL_SPEC_COST);
    ...
    bool DualSpecMgr::OnGossipSelect(Player* player, Creature* creature, uint32 sender, uint32 action, const std::string& code, uint32 gossipListId)
    {
        if (sWorld.getConfig(CONFIG_BOOL_DUAL_SPEC_ENABLED))
        {
            if (player && creature)
            {
                ...
                if (!code.empty())
                {
                    ...
                    {
                        SetPlayerSpecName(player, 2, strCode);
                    }
                    else if (action == GOSSIP_ACTION_INFO_DEF + 13)
                    {
                        SetPlayerSpecName(player, 3, strCode);
                    }
                ...
                switch (action)
                {
                    ...
                    {
                        if (GetPlayerActiveSpec(playerId) == 2)
                        {
                            player->GetPlayerMenu()->CloseGossip();
                            player->GetSession()->SendNotification(player->GetSession()->GetMangosString(DUAL_SPEC_ALREADY_ON_SPEC));
                            OnGossipSelect(player, creature, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 5, "", gossipListId);
                    ...
                    {
                        if (GetPlayerActiveSpec(playerId) == 3)
                        {
                            player->GetPlayerMenu()->CloseGossip();
                            player->GetSession()->SendNotification(player->GetSession()->GetMangosString(DUAL_SPEC_ALREADY_ON_SPEC));
                            OnGossipSelect(player, creature, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 5, "", gossipListId);
    ...
    bool DualSpecMgr::OnGossipSelect(Player* player, Item* item, uint32 sender, uint32 action, const std::string& code, uint32 gossipListId)
    {
        if (sWorld.getConfig(CONFIG_BOOL_DUAL_SPEC_ENABLED))
        {
            if (player)
    ...
    void DualSpecMgr::OnLearnTalent(Player* player, uint32 spellId)
    {
        if (sWorld.getConfig(CONFIG_BOOL_DUAL_SPEC_ENABLED))
        {
            if (player)
    ...
    void DualSpecMgr::OnResetTalents(Player* player, uint32 cost)
    {
        if (sWorld.getConfig(CONFIG_BOOL_DUAL_SPEC_ENABLED))
        {
            if (player)
    ...
    void DualSpecMgr::OnPreLoadFromDB(uint32 playerId)
    {
        if (sWorld.getConfig(CONFIG_BOOL_DUAL_SPEC_ENABLED))
        {
#ifdef ENABLE_PLAYERBOTS
    ...
    void DualSpecMgr::OnLoadFromDB(Player* player)
    {
        if (sWorld.getConfig(CONFIG_BOOL_DUAL_SPEC_ENABLED))
        {
#ifdef ENABLE_PLAYERBOTS
    ...
    void DualSpecMgr::OnSaveToDB(Player* player)
    {
        if (sWorld.getConfig(CONFIG_BOOL_DUAL_SPEC_ENABLED))
        {
#ifdef ENABLE_PLAYERBOTS
    ...
    void DualSpecMgr::OnDeleteFromDB(uint32 playerId)
    {
        if (sWorld.getConfig(CONFIG_BOOL_DUAL_SPEC_ENABLED))
        {
#ifdef ENABLE_PLAYERBOTS
    ...
    void DualSpecMgr::OnLogOut(Player* player)
    {
        if (sWorld.getConfig(CONFIG_BOOL_DUAL_SPEC_ENABLED))
        {
            if (player)
    ...
    void DualSpecMgr::OnCharacterCreated(Player* player)
    {
        if (sWorld.getConfig(CONFIG_BOOL_DUAL_SPEC_ENABLED))
        {
            if (player)
    ...
    bool DualSpecMgr::OnLoadActionButtons(Player* player, ActionButtonList& actionButtons)
    {
        if (sWorld.getConfig(CONFIG_BOOL_DUAL_SPEC_ENABLED))
        {
            if (player)
    ...
    bool DualSpecMgr::OnSaveActionButtons(Player* player, ActionButtonList& actionButtons)
    {
        if (sWorld.getConfig(CONFIG_BOOL_DUAL_SPEC_ENABLED))
        {
            if (player)
    ...
    DualSpecPlayerTalentMap* DualSpecMgr::GetPlayerTalents(uint32 playerId, int8 spec, bool assert)
    {
